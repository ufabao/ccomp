...
use self::assemblyast::StaticVariable;

use self::assemblyast::{ConditionalCode, Function, Instruction, Operand, Program, Register};
use self::tacky_to_asm::{convert_binary_op, convert_unary_op, convert_val};
use crate::tacky;
use crate::tacky::Tac;

fn convert_function(function: &tacky::Function) -> assemblyast::Function {
  Function {
...
fn convert_static_variable(static_var: &tacky::StaticVariable) -> assemblyast::StaticVariable {
  StaticVariable {
    name: static_var.name.clone(),
    global: static_var.global,
    value: static_var.init,
  }
}
  ...
  let top_level = tacky
    ...
    .map(|toplevel| match toplevel {
      tacky::TopLevel::Function(function) => {
        assemblyast::TopLevel::Function(convert_function(function))
      }
      tacky::TopLevel::StaticVariable(static_var) => {
        assemblyast::TopLevel::StaticVariable(convert_static_variable(static_var))
      }
    })
    .collect();
